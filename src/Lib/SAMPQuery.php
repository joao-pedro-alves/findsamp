...
class SAMPQuery
{
	...
	public function getInfo()
	{
		...
		$aDetails['hostname'] = utf8_encode((string) fread($this->rSocket, $iStrlen));
		
		$iStrlen = ord(fread($this->rSocket, 4));
		$aDetails['gamemode'] = utf8_encode((string) fread($this->rSocket, $iStrlen));
		
		$iStrlen = ord(fread($this->rSocket, 4));
		$aDetails['language'] = utf8_encode((string) fread($this->rSocket, $iStrlen));
